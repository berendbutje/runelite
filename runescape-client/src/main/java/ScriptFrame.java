...
public class ScriptFrame {
	...
	static int method1118(int opcode, Script var1, boolean var2) {
		String var7;
		if (opcode == ScriptOpcodes.MES) { // L: 1412
			...
			return 1; // L: 1415
		} else if (opcode == ScriptOpcodes.ANIM) {
			...
			return 1; // L: 1420
		} else if (opcode == ScriptOpcodes.IF_CLOSE) {
			...
			int var16;
			if (opcode == ScriptOpcodes.RESUME_COUNTDIALOG) {
				...
				PacketBufferNode var14 = AbstractWorldMapData.getPacketBufferNode(ClientPacket.SUBMIT_COUNT, Client.packetWriter.isaacCipher); // L: 1433
				var14.packetBuffer.writeInt(var16); // L: 1434
				Client.packetWriter.addNode(var14); // L: 1435
				return 1; // L: 1436
			} else {
				PacketBufferNode var12;
				if (opcode == ScriptOpcodes.RESUME_NAMEDIALOG) {
					...
					return 1; // L: 1445
				} else if (opcode == ScriptOpcodes.RESUME_STRINGDIALOG) {
					...
					String var4;
					int var10;
					if (opcode == ScriptOpcodes.OPPLAYER) {
						...
						return 1; // L: 1460
					} else if (opcode == ScriptOpcodes.IF_DRAGPICKUP) {
						...
						return 1; // L: 1469
					} else if (opcode == ScriptOpcodes.CC_DRAGPICKUP) {
						...
						return 1; // L: 1490
					} else if (opcode == ScriptOpcodes.OPENURL) {
						...
						return 1; // L: 1496
					} else if (opcode == ScriptOpcodes.RESUME_OBJDIALOG) {
						...
						return 1; // L: 1504
					} else if (opcode == ScriptOpcodes.BUG_REPORT) {
						...
						return 1; // L: 1585
					} else if (opcode == ScriptOpcodes.GETCANVASSIZE) {
						...
						return 1; // L: 1590
					} else if (opcode == ScriptOpcodes.MOBILE_SETFPS) {
						--class14.Interpreter_intStackSize; // L: 1593
						return 1; // L: 1594
					} else if (opcode == ScriptOpcodes.MOBILE_OPENSTORE) {
						return 1; // L: 1597
					} else if (opcode == ScriptOpcodes.MOBILE_OPENSTORECATEGORY) {
						...
						boolean var11;
						if (opcode == ScriptOpcodes.SETHIDEUSERNAME) {
							...
							return 1; // L: 1815
						} else if (opcode != 3183 && opcode != 3184) {
							return 2; // L: 1821
						} else {
